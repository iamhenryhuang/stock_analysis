...
def strategy(df, total, ma_num, stop_earn):
        ...
        if state == 'wait_long':
            ...
            if close < ma or close >= entry_price * (1 + stop_earn):
                ...
                if profit > best_profit:
                    best_profit = profit
                    best_buy = (buy_date, entry_price)
                    best_sell = (sell_date, close)
    ...
    if shares > 0:
        ...
        if profit > best_profit:
            best_profit = profit
            best_buy = (buy_date, entry_price)
            best_sell = (sell_date, df['Close'].iloc[-1])

    print(f"Best Buy Point: {best_buy}")
    print(f"Best Sell Point: {best_sell}")
...
try:
    ...
    best_buy, best_sell = strategy(df_btc, total, ma_num, stop_earn)
    
    # 繪製 價格 & 均線圖
    df_btc['MA'] = df_btc['Close'].rolling(window=ma_num).mean()
    selected_data = df_btc.loc['2024-06-04':'2024-07-04', ['Close', 'MA']]
